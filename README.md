...
1. Install [Cursor IDE](https://www.cursor.com/), **select any options during installation** (they don't matter), then log in. If you had it installed before, `Cmd/Ctrl+Shift+P` "Attempt Update" to get the latest update. This tutorial assumes you use either Cursor's free trial (first two weeks on a new account) or Cursor Pro. Yes, this tutorial promotes a paid product
